...
export default function SimulationDashboard() {
  ...
  useEffect(() => {
    if (step > 0 && step % 5 === 0 && Date.now() - lastCommentaryTime > 2000) {
      ...
      if (currentData) {
        fetchAeroAnalysis(currentData).then(res => {
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <p style={{ color: 'var(--text-secondary)', lineHeight: '1.6' }}>
              {latestCommentary || 'Start simulation to see AI analysis...'}
            </p>
          </div>
            ...
            <div>
              {analyses.map((analysis, i) => (
                <div key={i} style={{
                  ...
                  <strong>Step {i + 1}:</strong> {analysis ? analysis.substring(0, 100) + '...' : 'Loading...'}
                </div>
              ))}
            </div>
          </div>
